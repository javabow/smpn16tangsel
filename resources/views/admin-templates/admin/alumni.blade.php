...
@section('title')
Admin Page SMKN 16 Tangerang Selatan | Staff - Alumni
@endsection
@section('title-header')
  Staff Management - Alumni
@endsection
@section('content')
  <div id="content-cont" class="container bg-light p-4">
    @include('admin-templates.alert-box')
    <p>
      <a class="btn btn-info" href="{{ url('admin/alumni/create') }}">Add Alumni <i class="fas fa-plus"></i></a>
    </p>
      ...
      <tbody>
        @foreach ($staff as $s)
          <tr val="{{ $s->id }}">
            ...
            <td>{{ $s->updated_at }}</td>
            <td class="row">
              <form class="col-md-6 p-0" action="{{ url('admin/alumni/'.$s->id.'/edit') }}" method="get">
                <button type="submit" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fas fa-edit"></i> </button>
              </form>
              <form id="delete" class="col-md-6 p-0" action="{{ url('admin/alumni/'.$s->id) }}" method="post">
