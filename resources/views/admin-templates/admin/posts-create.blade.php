...
@section('title')
Admin Page SMKN 16 Tangerang Selatan | Posts - Create
@endsection
@section('css')
        ...
        @foreach ($categories as $category)
          ...
          @endfor
            <li class="list-group-item" val="{{ $category->id }}">
                <input class="mr-2" id="cat-{{$category->id}}" name="cat[]" type="checkbox" value="{{ $category->id.'-'.$category->name }}">
                <label for="cat-{{$category->id}}"> {!! $listOpen . $category->name. ' id : ('.$category->id.')'. $listClose !!}</label>
            </li>
        @endforeach
      </ul>
